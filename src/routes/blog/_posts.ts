  ...
  return files
    ...
    .map((p) => {
      return {
        ...
        metadata: {
          ...p.metadata,
          humanDate: new Date(p.metadata.date).toLocaleDateString(undefined, {
            year: "numeric",
            month: "long",
            day: "2-digit",
          }),
...
function parseFileToHtmlAndMeta(
  ...
  const assetsSrc = "\\" + dirname(file).split(sep).slice(1).join("/");
  ...
  renderer.code = (source, lang) => {
    ...
    const codeBlock = highlightedLines
    ...
    const headingParts = [file, ...createHeadingParts(metadata)].filter(
      Boolean
    );
    const heading = headingParts.length
      ? `<div class="code-heading">${headingParts.join("•")}</div>`
      : "";
    return `<pre class='language-${prismLanguage}' tabindex="0">${heading}${codeBlock}</pre>`;
  };
  ...
  renderer.heading = (text, level, rawtext) => {
    ...
    return `
      <h${level} id="${fragment}">
        <a href="#${fragment}" class="anchor" aria-hidden="true" tabindex="-1">${headingText}</a>
      </h${level}>`;
  };
