...
```html [title=Template] [linenumber=6,11]
<form novalidate>
	<fieldset>
		<legend>Customer Information</legend>
		<div>
			<label for="firstName">First Name</label>
			<input id="firstName" type="text" [control]="customerForm.firstName" />
		</div>

		<div>
			<label for="lastName">Last Name</label>
			<input id="lastName" type="text" [control]="customerForm.lastName" />
		</div>
	</fieldset>
</form>
```

```ts [title=Component] [linenumber=2]
@Component({
	imports: [Control],
})
export class CustomerForm {
...
Via the `path` variable you can access all the form controls and groups to define validation rules.
...
To check the validatity of a control, use the `valid`, `invalid`, and `pending` (useful for async validation) signals.
...
- The `disabled` and `readonly` states are reflected as attributes on the input element. If these conditions are set to a parent group, they are applied to all child controls. The `hidden` state does not affect the input element, it's up to you to hide the element in the UI based on this state
...
```typescript [linenumber=9-10, 12-16]
@Component({...})
export class CustomerForm {
	protected readonly customerForm = form(signal(this.initializeCustomerModel()), (path) => {
		required(path.firstName);
...
<div>
	<label
		>Street
		<input type="text" [control]="address().street" />
	</label>
</div>
...
<div>
	<label>
		City
		<input type="text" [control]="address().city" />
	</label>
</div>
...
<div>
	<label
		>Zip Code
		<input type="text" [control]="address().zipCode" />
	</label>
</div>
...
```angular-html [linenumber=22-27, 35-45]
<form novalidate>
	...
	<fieldset>
		...
		<div>
			<label for="firstName">First Name</label>
			<input type="text" [control]="customerForm.firstName" />
		</div>
	...
	<div>
		<input type="checkbox" id="hasBillingAddress" [control]="customerForm.hasBillingAddress" />
	...
	<div>
		<input type="checkbox" id="hasShippingAddress" [control]="customerForm.hasShippingAddress" />
		...
		<fieldset>
			...
			<sandbox-customer-address [address]="customerForm.shippingAddress">
				<div>
					...
					<textarea id="note" [control]="customerForm.shippingAddress.note"></textarea>
				</div>
...
If the HTTP call is successful, the form is reset to its initial state using the `reset()` method, this resets all control states (touched, dirty, validations) of the form.
...
- [Signal Forms Custom Controls — Simplifying ControlValueAccessor (YouTube)](https://www.youtube.com/watch?v=gvL_mTQZIYE), by Dmytro Mezhenskyi
