<script lang="ts">
	...
	let divElement: HTMLElement | undefined = $state();

	onMount(() => {
		if (!divElement) return;
		const commentsElement = divElement;
		...
		const observer = new IntersectionObserver(
			([entry]) => {
				...
				commentsElement.appendChild(scriptElm);

				unsubscribe = theme.subscribe(updateTheme);
			},
			{ rootMargin: '500px' },
		);
